...
/*
int main(){
	pthread_t t_serveur_udp;
	pthread_t t_client_udp;
	
	printf("Thread créés\n");
	
	pthread_create(&t_serveur_udp, NULL, serveur, NULL);
	pthread_create(&t_client_udp, NULL, recup_consigne, NULL);
	
	pthread_join(t_serveur_udp, NULL);
...
{	
	...
	pthread_t t[3]; // création du thread moteur
	...
	pthread_create(&t_consigne, &attr[0],actualiser_consigne, NULL);

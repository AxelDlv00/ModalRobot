...
resize(frame, frame, Size(), 0.2, 0.02, INTER_LINEAR);
...
// Convertissez l'image modifiée de retour en BGR
Mat imageBGR;
cvtColor(frame_HSV, frame, COLOR_HSV2BGR);
...
Mat gray;
cvtColor(frame_threshold, gray, COLOR_BGR2GRAY);
Mat thresh;
threshold(gray, thresh, 127, 255, THRESH_BINARY);
vector<vector<Point>> contours;
vector<Vec4i> hierarchy;
findContours(thresh, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE);
Mat imgContours = Mat::zeros(frame.size(), frame.type());
